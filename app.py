...
# Groq's model catalog changes over time. Keep the model configurable and use
# a current production model by default instead of hard-coding an older model.
...
def get_llm():
    ...
    return FakeMessagesListChatModel(
        responses=[AIMessage(content="Provider not configured correctly. Connect an OpenAI or Groq key.")]
    )
...
def llm_config_sidebar():
    ...
    st.sidebar.subheader("Active Buddy")
    ...
    if agent_mode == "Professor Z":
        ...
        st.session_state.current_personality = (
            "You are Professor Z, a patient academic mentor. "
            "Don't give answers directly; guide the user with analogies."
        )
    else:
        st.sidebar.warning("Rival Mode")
        st.session_state.current_personality = (
            "You are Aman, a sharp technical rival. "
            "Point out bugs bluntly and push the user to be better."
        )
    ...
    if provider == "Groq":
        ...
        if st.sidebar.button("Connect OpenAI"):
            if not api_key.strip():
                st.sidebar.error("Enter an OpenAI API key first.")
            else:
